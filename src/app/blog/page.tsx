          ...
          {BLOG_POSTS.map((post) => (
            <Link key={post.slug} href={`/blog/${post.slug}`} className="card-base p-6 group hover:-translate-y-1.5 transition-transform duration-200 block">
              <div className="flex items-center gap-2 mb-3">
                <span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-[#136F63]/10 text-[#136F63]">{post.category}</span>
                <span className="text-xs text-[#6B6B6E] flex items-center gap-1"><Clock size={11} /> {post.readTime}</span>
              </div>
              <h2 className="font-bold text-lg text-[#1C1C1E] mb-2 group-hover:text-[#136F63] transition-colors leading-snug">{post.title}</h2>
