          ...
          {SERVICES.map((service) => (
            <div key={service.id} className="card-base p-6 group hover:-translate-y-1.5 transition-transform duration-200">
              <Link href={`/services/${service.slug}`}>
                <div className="w-12 h-12 rounded-2xl flex items-center justify-center mb-4 text-[#1f2937]" style={{ background: "linear-gradient(135deg, #FFC107, #F5B600)" }}>
                  {ICON_MAP[service.icon]}
                </div>
                <h2 className="font-bold text-xl text-[#1f2937] mb-2 group-hover:text-[#FFC107] transition-colors">{service.title}</h2>
                <p className="text-sm text-[#6B6B6E] mb-4 leading-relaxed">{service.description}</p>
                <div className="flex items-center justify-between">
                  <span className="text-sm font-semibold text-[#FFC107] bg-[#FFC107]/10 px-3 py-1 rounded-lg">From {service.startingPrice}</span>
                  <ArrowRight size={16} className="text-[#FFC107] transition-transform group-hover:translate-x-1" />
                </div>
              </Link>
            </div>
          ))}
