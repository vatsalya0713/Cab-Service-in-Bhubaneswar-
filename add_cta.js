...
function processDir(dir) {
  ...
  for (const file of files) {
    ...
    if (stat.isDirectory()) {
      processDir(fullPath);
    } else if (file === 'page.tsx' && fullPath.includes('destinations')) {
      let content = fs.readFileSync(fullPath, 'utf8');
      if (content.includes('lg:col-span-2 space-y-8')) {
        // Find the closing div of lg:col-span-2 space-y-8
        // It's followed by <div>\n<div className="sticky top-24">
        if (!content.includes('Ready to book this trip?')) {
            content = content.replace(/          <\/div>\s*<div>\s*<div className="sticky top-24">/g, ctaHTML + '\n          <div>\n            <div className="sticky top-24">');
...
processDir('src/app/destinations');
